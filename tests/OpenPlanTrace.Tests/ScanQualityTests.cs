...
public sealed class ScanQualityTests
{
    ...
    [Fact]
    public void PlacementExporter_PreservesSourceBackedExteriorFallbackBesideCleanExteriorSpan()
    {
        var primaryWall = SyntheticWall("primary-exterior", 80, 100, 260, 100) with
        {
            DetectionKind = WallDetectionKind.ParallelLinePair,
            WallType = WallType.Exterior,
            PairEvidence = new WallPairEvidence(
                new PlanLineSegment(new PlanPoint(80, 97), new PlanPoint(260, 97)),
                new PlanLineSegment(new PlanPoint(80, 103), new PlanPoint(260, 103)),
                FaceSeparation: 6,
                OverlapRatio: 1,
                Score: 0.90,
                FirstFaceFragmentCount: 8,
                SecondFaceFragmentCount: 9,
                FirstFaceSourcePrimitiveIds: ["primary-face-a"],
                SecondFaceSourcePrimitiveIds: ["primary-face-b"]),
            Evidence =
            [
                "parallel wall-face pair",
        ...
        var fallbackWall = SyntheticWall("fallback-exterior", 80, 106, 260, 106) with
        {
            DetectionKind = WallDetectionKind.ParallelLinePair,
            WallType = WallType.Exterior,
            PairEvidence = new WallPairEvidence(
                new PlanLineSegment(new PlanPoint(80, 103), new PlanPoint(260, 103)),
                new PlanLineSegment(new PlanPoint(80, 109), new PlanPoint(260, 109)),
                FaceSeparation: 6,
                OverlapRatio: 1,
                Score: 0.88,
        ...
        var wallGraph = new WallGraph(
            [SyntheticNode("n1", 80, 100), SyntheticNode("n2", 260, 100)],
            [SyntheticEdge("e1", "n1", "n2", primaryWall.Id)],
            [
                new WallGraphComponent(
                    "main-exterior-component",
                    1,
                    WallGraphComponentKind.MainStructural,
                    new PlanRect(77, 97, 186, 12),
                    [primaryWall.Id, fallbackWall.Id],
                    ["n1", "n2"],
                    ["e1"],
                    [primaryWall.Id, fallbackWall.Id],
                    primaryWall.DrawingLength + fallbackWall.DrawingLength,
                    Confidence.High,
                    ["synthetic main exterior shell component"])
            ]);
        var result = CreateSyntheticResult(
            walls: [primaryWall, fallbackWall],
            wallGraph: wallGraph,
            wallEvidenceMap: new WallEvidenceMap(
                [],
                [],
                [
                    StrongPairedWallEvidence(primaryWall, "both endpoints supported by structural context"),
                    StrongPairedWallEvidence(fallbackWall, "both endpoints supported by structural context")
                ])) with
        {
            Quality = UsableQuality()
        };
        ...
        var placementJson = PlanPlacementJsonExporter.Serialize(
            ...
            new PlanPlacementJsonExportOptions { WriteIndented = false });
        using var document = JsonDocument.Parse(placementJson);
        var exportedFallback = document.RootElement.GetProperty("walls").EnumerateArray().Single(item =>
            item.GetProperty("id").GetString() == fallbackWall.Id);
        var fallbackSpans = exportedFallback.GetProperty("topologySpans").EnumerateArray().ToArray();

        Assert.True(exportedFallback.GetProperty("reliability").GetProperty("readyForCoordinatePlacement").GetBoolean());
        Assert.Equal(JsonValueKind.Null, exportedFallback.GetProperty("placementOmission").ValueKind);
        Assert.Contains(fallbackSpans, span =>
            span.GetProperty("id").GetString()?.Contains(":source-backed-fallback:", StringComparison.Ordinal) == true);
    }

    [Fact]
    public void PlacementExporter_DoesNotLetLongExteriorRunSwallowShortExteriorWallSpan()
    {
        var longExteriorWall = SyntheticWall("long-exterior", 80, 100, 520, 100) with
        {
            DetectionKind = WallDetectionKind.ParallelLinePair,
            WallType = WallType.Exterior,
            PairEvidence = new WallPairEvidence(
                ...
                FirstFaceSourcePrimitiveIds: ["long-face-a"],
                SecondFaceSourcePrimitiveIds: ["long-face-b"]),
            Evidence =
            [
                "parallel wall-face pair",
                "face separation 6 drawing units",
        ...
        var localExteriorWall = SyntheticWall("local-exterior", 200, 106, 290, 106) with
        {
            DetectionKind = WallDetectionKind.ParallelLinePair,
            WallType = WallType.Exterior,
            PairEvidence = new WallPairEvidence(
                new PlanLineSegment(new PlanPoint(200, 103), new PlanPoint(290, 103)),
                new PlanLineSegment(new PlanPoint(200, 109), new PlanPoint(290, 109)),
                FaceSeparation: 6,
                OverlapRatio: 1,
                Score: 0.89,
                FirstFaceFragmentCount: 3,
                SecondFaceFragmentCount: 3,
                FirstFaceSourcePrimitiveIds: ["local-face-a"],
                SecondFaceSourcePrimitiveIds: ["local-face-b"]),
            Evidence =
            [
                "parallel wall-face pair",
                "face separation 6 drawing units",
        ...
        var wallGraph = new WallGraph(
            [
                SyntheticNode("n1", 80, 100),
                SyntheticNode("n2", 520, 100),
                SyntheticNode("n3", 200, 106),
                SyntheticNode("n4", 290, 106)
            ],
            [
                SyntheticEdge("e1", "n1", "n2", longExteriorWall.Id),
                SyntheticEdge("e2", "n3", "n4", localExteriorWall.Id)
            ],
            [
                new WallGraphComponent(
                    "main-exterior-component",
                    ...
                    [longExteriorWall.Id, localExteriorWall.Id],
                    ["n1", "n2", "n3", "n4"],
                    ["e1", "e2"],
                    [longExteriorWall.Id, localExteriorWall.Id],
                    longExteriorWall.DrawingLength + localExteriorWall.DrawingLength,
                    Confidence.High,
                    ["synthetic main exterior shell component"])
            ]);
        var result = CreateSyntheticResult(
            walls: [longExteriorWall, localExteriorWall],
            wallGraph: wallGraph,
            wallEvidenceMap: new WallEvidenceMap(
                [],
                [],
                [
                    StrongPairedWallEvidence(longExteriorWall, "both endpoints supported by structural context"),
                    StrongPairedWallEvidence(localExteriorWall, "both endpoints supported by structural context")
                ])) with
        {
            Quality = UsableQuality()
        };
        ...
        var placementJson = PlanPlacementJsonExporter.Serialize(
            ...
            new PlanPlacementJsonExportOptions { WriteIndented = false });
        using var document = JsonDocument.Parse(placementJson);
        var exportedLocalWall = document.RootElement.GetProperty("walls").EnumerateArray().Single(item =>
            item.GetProperty("id").GetString() == localExteriorWall.Id);
        var localSpans = exportedLocalWall.GetProperty("topologySpans").EnumerateArray().ToArray();

        Assert.True(exportedLocalWall.GetProperty("reliability").GetProperty("readyForCoordinatePlacement").GetBoolean());
        Assert.Equal(JsonValueKind.Null, exportedLocalWall.GetProperty("placementOmission").ValueKind);
        Assert.Single(localSpans);
        Assert.Equal("local-exterior", localSpans[0].GetProperty("wallId").GetString());
    }
    ...
    [Fact]
    public void PlacementExporter_AllowsTrustedLongSecondaryInteriorBodyWithoutRoomBoundarySupport()
    {
        var regions = new[]
        {
            new SheetRegion("sheet", 1, RegionKind.Sheet, new PlanRect(0, 0, 700, 500), Confidence.High),
            new SheetRegion("main", 1, RegionKind.MainFloorPlan, new PlanRect(40, 40, 540, 380), Confidence.High)
        };
        var trustedWall = SyntheticWall("trusted-secondary-interior", 180, 180, 430, 180) with
        {
            DetectionKind = WallDetectionKind.ParallelLinePair,
            WallType = WallType.Interior,
            PairEvidence = new WallPairEvidence(
                new PlanLineSegment(new PlanPoint(180, 177), new PlanPoint(430, 177)),
                new PlanLineSegment(new PlanPoint(180, 183), new PlanPoint(430, 183)),
                FaceSeparation: 6,
                OverlapRatio: 1,
                Score: 0.93,
                FirstFaceFragmentCount: 22,
                SecondFaceFragmentCount: 35,
                FirstFaceSourcePrimitiveIds: ["trusted-secondary-face-a"],
                SecondFaceSourcePrimitiveIds: ["trusted-secondary-face-b"]),
            SourcePrimitiveIds = Enumerable.Range(1, 53).Select(index => $"trusted-secondary-source-{index}").ToArray(),
            Evidence =
            [
                "parallel wall-face pair",
                "face separation 6 drawing units",
                ...
                "second face merged 35 fragments",
        ...
        var shortReviewWall = SyntheticWall("secondary-short-review-piece", 150, 180, 164, 180) with
        {
        ...
        var roomAnchor = SyntheticWall("room-boundary-anchor", 100, 100, 240, 100);
        var wallGraph = new WallGraph(
            [SyntheticNode("n0", 150, 180), SyntheticNode("n1", 164, 180), SyntheticNode("n2", 180, 180), SyntheticNode("n3", 430, 180)],
            [
                SyntheticEdge("e0", "n0", "n1", shortReviewWall.Id),
                SyntheticEdge("e1", "n2", "n3", trustedWall.Id)
            ],
            [
                new WallGraphComponent(
                    "secondary-long-interior-component",
                    1,
                    WallGraphComponentKind.SecondaryStructural,
                    new PlanRect(150, 177, 283, 6),
                    [shortReviewWall.Id, trustedWall.Id],
                    ["n0", "n1", "n2", "n3"],
                    ["e0", "e1"],
                    [shortReviewWall.Id, trustedWall.Id],
                    trustedWall.DrawingLength + shortReviewWall.DrawingLength,
                    Confidence.High,
                    ["synthetic secondary component with one long paired wall and one short review fragment"])
            ]);
        var shortReviewAssessment = new WallEvidenceWallAssessment(
            shortReviewWall.Id,
            shortReviewWall.PageNumber,
            shortReviewWall.Bounds,
            WallEvidenceCategory.RecoveredWallBody,
            new Confidence(0.78),
            PlacementReady: false,
            RequiresReview: true,
            RejectedAsNoise: false,
            shortReviewWall.SourcePrimitiveIds,
            shortReviewWall.Evidence)
        {
            Decision = WallEvidenceDecision.Review,
            ScoreBreakdown = new WallEvidenceScoreBreakdown(
                ...
                0,
                0,
                ["recovered parallel-face wall band", "both endpoints supported by structural context"],
                ["not placement-ready without review"])
        };
        var trustedAssessment = new WallEvidenceWallAssessment(
            trustedWall.Id,
            trustedWall.PageNumber,
            trustedWall.Bounds,
            WallEvidenceCategory.StrongWallBody,
            Confidence.High,
            PlacementReady: true,
            RequiresReview: false,
            RejectedAsNoise: false,
            trustedWall.SourcePrimitiveIds,
            trustedWall.Evidence)
        {
            Decision = WallEvidenceDecision.Accept,
            ScoreBreakdown = new WallEvidenceScoreBreakdown(
                0.90,
                ...
                0,
                0,
                ["strong parallel-face wall pair", "supported wall evidence inside exterior envelope"],
                [])
        };
        var result = CreateSyntheticResult(
            regions: regions,
            walls: [shortReviewWall, trustedWall, roomAnchor],
            wallGraph: wallGraph,
            rooms: [SyntheticRoom("r1", new PlanRect(100, 100, 140, 120), [roomAnchor.Id])],
            wallEvidenceMap: new WallEvidenceMap(
                [],
                [],
                [
                    shortReviewAssessment,
                    trustedAssessment,
                    StrongPairedWallEvidence(roomAnchor, "both endpoints supported by structural context")
                ])) with
        {
        ...
        using var document = JsonDocument.Parse(placementJson);
        var exportedWall = document.RootElement.GetProperty("walls").EnumerateArray().Single(item =>
            item.GetProperty("id").GetString() == trustedWall.Id);
        var exportedShortWall = document.RootElement.GetProperty("walls").EnumerateArray().Single(item =>
            item.GetProperty("id").GetString() == shortReviewWall.Id);

        Assert.DoesNotContain(trustedWall.Id, reasons.Keys);
        Assert.Contains(shortReviewWall.Id, reasons.Keys);
        Assert.Contains(
            WallPlacementContextGuards.SecondaryStructuralWithoutRoomBoundarySupportReason,
            reasons[shortReviewWall.Id]);
        Assert.True(exportedWall.GetProperty("reliability").GetProperty("readyForCoordinatePlacement").GetBoolean());
        Assert.NotEmpty(exportedWall.GetProperty("topologySpans").EnumerateArray());
        Assert.Equal(JsonValueKind.Null, exportedWall.GetProperty("placementOmission").ValueKind);
        Assert.False(exportedShortWall.GetProperty("reliability").GetProperty("readyForCoordinatePlacement").GetBoolean());
        Assert.Equal(
            "secondary_without_room_boundary_support",
            exportedShortWall.GetProperty("placementOmission").GetProperty("code").GetString());
    }

    [Fact]
    public void PlacementExporter_BlocksTrustedLongSecondaryInteriorBodyNearCoveredEntryDetail()
    {
        var regions = new[]
        {
            new SheetRegion("sheet", 1, RegionKind.Sheet, new PlanRect(0, 0, 700, 500), Confidence.High),
        ...
        var coveredWall = SyntheticWall("covered-secondary-interior-detail", 180, 180, 430, 180) with
        {
            DetectionKind = WallDetectionKind.ParallelLinePair,
            WallType = WallType.Interior,
            PairEvidence = new WallPairEvidence(
                ...
                FirstFaceFragmentCount: 22,
                SecondFaceFragmentCount: 35,
                FirstFaceSourcePrimitiveIds: ["covered-secondary-face-a"],
                SecondFaceSourcePrimitiveIds: ["covered-secondary-face-b"]),
            SourcePrimitiveIds = Enumerable.Range(1, 53).Select(index => $"covered-secondary-source-{index}").ToArray(),
            Evidence =
            [
                "parallel wall-face pair",
                "face separation 6 drawing units",
        ...
        var roomAnchor = SyntheticWall("room-boundary-anchor", 100, 100, 240, 100);
        var wallGraph = new WallGraph(
            [SyntheticNode("n1", 180, 180), SyntheticNode("n2", 430, 180)],
            [SyntheticEdge("e1", "n1", "n2", coveredWall.Id)],
            [
                new WallGraphComponent(
                    "secondary-covered-interior-component",
                    1,
                    WallGraphComponentKind.SecondaryStructural,
                    new PlanRect(177, 177, 256, 6),
                    [coveredWall.Id],
                    ["n1", "n2"],
                    ["e1"],
                    [coveredWall.Id],
                    coveredWall.DrawingLength,
                    Confidence.High,
                    ["synthetic covered-entry long secondary interior detail"])
            ]);
        var result = CreateSyntheticResult(
            regions: regions,
            walls: [coveredWall, roomAnchor],
            wallGraph: wallGraph,
            rooms: [SyntheticRoom("r1", new PlanRect(100, 100, 140, 120), [roomAnchor.Id])],
            wallEvidenceMap: new WallEvidenceMap(
                [],
                [],
                [
                    StrongPairedWallEvidence(coveredWall, "both endpoints supported by structural context"),
                    StrongPairedWallEvidence(roomAnchor, "both endpoints supported by structural context")
                ])) with
        {
            Quality = UsableQuality()
        ...
        using var document = JsonDocument.Parse(placementJson);
        var exportedWall = document.RootElement.GetProperty("walls").EnumerateArray().Single(item =>
            item.GetProperty("id").GetString() == coveredWall.Id);

        Assert.Contains(coveredWall.Id, reasons.Keys);
        Assert.Contains(
            WallPlacementContextGuards.SecondaryStructuralWithoutRoomBoundarySupportReason,
            reasons[coveredWall.Id]);
        Assert.False(exportedWall.GetProperty("reliability").GetProperty("readyForCoordinatePlacement").GetBoolean());
        Assert.Equal(
            "secondary_without_room_boundary_support",
            exportedWall.GetProperty("placementOmission").GetProperty("code").GetString());
    ...
    [Fact]
    public void PlacementExporter_BlocksRiskyMainStructuralInteriorWithoutSemanticSupport()
    {
        var regions = new[]
        {
            new SheetRegion("sheet", 1, RegionKind.Sheet, new PlanRect(0, 0, 600, 400), Confidence.High),
        ...
        var roomBoundary = SyntheticWall("room-boundary", 120, 100, 260, 100) with
        {
            DetectionKind = WallDetectionKind.ParallelLinePair,
            WallType = WallType.Interior,
            Evidence = ["parallel wall-face pair", "wall evidence: strong double-edge wall body"]
        };
        var suspiciousWall = SyntheticWall("unconfirmed-main-interior", 180, 180, 310, 180) with
        {
            DetectionKind = WallDetectionKind.ParallelLinePair,
            WallType = WallType.Interior,
            SourcePrimitiveIds = Enumerable.Range(1, 34).Select(index => $"main-risk-source-{index}").ToArray(),
            Evidence =
            [
                "parallel wall-face pair",
                "first face merged 31 fragments",
                "second face merged 10 fragments",
                "layer (unlayered) classified Unknown (0.35)",
                "layer evidence: no strong layer name or geometry evidence",
                "wall evidence: strong double-edge wall body"
            ]
                ...
                [
                        ...
                        suspiciousWall.Evidence)
                    {
                        ...
                        ScoreBreakdown = new WallEvidenceScoreBreakdown(
                            ...
                            0,
                            0,
                            ["strong parallel-face wall pair", "both endpoints supported by structural context"],
                            [])
                    }
                ])) with
        {
        ...
        Assert.Equal(
            ...
            document.RootElement
                ...
                .GetProperty("main_structural_semantic_support_review_required")
                .GetInt32());
    }

    [Fact]
    public void PlacementExporter_AllowsTrustedLongMainStructuralInteriorBodyWithoutSemanticRoomBoundary()
    {
        var regions = new[]
        {
            new SheetRegion("sheet", 1, RegionKind.Sheet, new PlanRect(0, 0, 600, 400), Confidence.High),
        ...
        var roomBoundary = SyntheticWall("trusted-room-boundary", 120, 100, 260, 100) with
        {
            DetectionKind = WallDetectionKind.ParallelLinePair,
            WallType = WallType.Interior,
            Evidence = ["parallel wall-face pair", "wall evidence: strong double-edge wall body"]
        };
        var trustedWall = SyntheticWall("trusted-main-interior-without-room-semantics", 180, 180, 330, 180) with
        {
            DetectionKind = WallDetectionKind.ParallelLinePair,
            WallType = WallType.Interior,
            PairEvidence = new WallPairEvidence(
                new PlanLineSegment(new PlanPoint(180, 176), new PlanPoint(330, 176)),
                new PlanLineSegment(new PlanPoint(180, 184), new PlanPoint(330, 184)),
                FaceSeparation: 8,
                OverlapRatio: 1,
                Score: 0.91,
                FirstFaceFragmentCount: 24,
                SecondFaceFragmentCount: 18,
                FirstFaceSourcePrimitiveIds: ["trusted-main-face-a"],
                SecondFaceSourcePrimitiveIds: ["trusted-main-face-b"]),
            SourcePrimitiveIds = Enumerable.Range(1, 42).Select(index => $"trusted-main-source-{index}").ToArray(),
            Evidence =
            [
                "parallel wall-face pair",
                "face separation 8 drawing units",
                "pair score 0.91",
                "overlap ratio 1",
                "first face merged 24 fragments",
                "second face merged 18 fragments",
                "layer (unlayered) classified Dimension (0.24)",
                "layer evidence: contains dimension-like text",
                "wall type interior: supported wall evidence inside exterior envelope",
                "wall evidence: strong double-edge wall body"
        ...
        var wallGraph = new WallGraph(
            [
                ...
                SyntheticNode("n2", 260, 100),
                SyntheticNode("n3", 180, 180),
                SyntheticNode("n4", 330, 180)
            ],
            [
                SyntheticEdge("e1", "n1", "n2", roomBoundary.Id),
                SyntheticEdge("e2", "n3", "n4", trustedWall.Id)
            ],
            [
                new WallGraphComponent(
                    "component-main",
                    1,
                    WallGraphComponentKind.MainStructural,
                    new PlanRect(116, 96, 218, 88),
                    [roomBoundary.Id, trustedWall.Id],
                    ["n1", "n2", "n3", "n4"],
                    ["e1", "e2"],
                    [roomBoundary.Id, trustedWall.Id],
                    290,
        ...
        var result = CreateSyntheticResult(
            regions: regions,
            walls: [roomBoundary, trustedWall],
            wallGraph: wallGraph,
            rooms: [SyntheticRoom("r1", new PlanRect(120, 100, 140, 120), [roomBoundary.Id])],
            wallEvidenceMap: new WallEvidenceMap(
                [],
    ...
    [Fact]
    public void PlacementExporter_AllowsNoisyVectorMainStructuralInteriorBodyWithEndpointSupport()
    {
        var regions = new[]
        {
            new SheetRegion("sheet", 1, RegionKind.Sheet, new PlanRect(0, 0, 600, 400), Confidence.High),
        ...
        var roomBoundary = SyntheticWall("noisy-room-boundary", 120, 100, 260, 100) with
        {
            DetectionKind = WallDetectionKind.ParallelLinePair,
            WallType = WallType.Interior,
            Evidence = ["parallel wall-face pair", "wall evidence: strong double-edge wall body"]
        };
        var noisyWall = SyntheticWall("noisy-main-interior-without-room-semantics", 180, 180, 330, 180) with
        {
            DetectionKind = WallDetectionKind.ParallelLinePair,
            WallType = WallType.Interior,
            PairEvidence = new WallPairEvidence(
                ...
                SecondFaceFragmentCount: 134,
                FirstFaceSourcePrimitiveIds: ["noisy-main-face-a"],
                SecondFaceSourcePrimitiveIds: ["noisy-main-face-b"]),
            SourcePrimitiveIds = Enumerable.Range(1, 52).Select(index => $"noisy-main-source-{index}").ToArray(),
            Evidence =
            [
                "parallel wall-face pair",
                "face separation 8 drawing units",
                ...
                "second face merged 134 fragments",
                "layer (unlayered) classified Dimension (0.24)",
                "layer evidence: contains dimension-like text",
                "wall type interior: supported wall evidence inside exterior envelope",
                "wall evidence: strong double-edge wall body"
        ...
        var wallGraph = new WallGraph(
            ...
            [
                SyntheticEdge("e1", "n1", "n2", roomBoundary.Id),
                SyntheticEdge("e2", "n3", "n4", noisyWall.Id)
            ],
            [
                new WallGraphComponent(
                    "component-main",
        ...
        var noisyAssessment = new WallEvidenceWallAssessment(
            noisyWall.Id,
            noisyWall.PageNumber,
            noisyWall.Bounds,
            WallEvidenceCategory.StrongWallBody,
            Confidence.High,
            PlacementReady: true,
        ...
        using var document = JsonDocument.Parse(placementJson);
        var exportedWall = document.RootElement.GetProperty("walls").EnumerateArray().Single(item =>
            item.GetProperty("id").GetString() == noisyWall.Id);

        Assert.DoesNotContain(noisyWall.Id, reasons.Keys);
        Assert.True(exportedWall.GetProperty("reliability").GetProperty("readyForCoordinatePlacement").GetBoolean());
        Assert.NotEmpty(exportedWall.GetProperty("topologySpans").EnumerateArray());
        Assert.Equal(JsonValueKind.Null, exportedWall.GetProperty("placementOmission").ValueKind);
    }
    ...
    [Fact]
    public void PlacementExporter_BlocksTrustedLongMainStructuralInteriorBodyNearCoveredArea()
    {
        var regions = new[]
        {
            new SheetRegion("sheet", 1, RegionKind.Sheet, new PlanRect(0, 0, 600, 400), Confidence.High),
        ...
        var roomBoundary = SyntheticWall("covered-guard-room-boundary", 120, 100, 260, 100) with
        {
            DetectionKind = WallDetectionKind.ParallelLinePair,
            WallType = WallType.Interior,
            Evidence = ["parallel wall-face pair", "wall evidence: strong double-edge wall body"]
        };
        var suspiciousWall = SyntheticWall("covered-area-main-interior", 180, 180, 330, 180) with
        {
            DetectionKind = WallDetectionKind.ParallelLinePair,
            WallType = WallType.Interior,
            PairEvidence = new WallPairEvidence(
                ...
                SecondFaceFragmentCount: 12,
                FirstFaceSourcePrimitiveIds: ["covered-main-face-a"],
                SecondFaceSourcePrimitiveIds: ["covered-main-face-b"]),
            SourcePrimitiveIds = Enumerable.Range(1, 32).Select(index => $"covered-main-source-{index}").ToArray(),
            Evidence =
            [
                "parallel wall-face pair",
                "face separation 8 drawing units",
        ...
        var suspiciousAssessment = new WallEvidenceWallAssessment(
            suspiciousWall.Id,
            suspiciousWall.PageNumber,
            suspiciousWall.Bounds,
            WallEvidenceCategory.StrongWallBody,
    ...
    [Fact]
    public void PlacementExporter_AllowsRiskyMainStructuralInteriorWhenRoomBoundaryConfirmsIt()
    {
        var regions = new[]
        {
            new SheetRegion("sheet", 1, RegionKind.Sheet, new PlanRect(0, 0, 600, 400), Confidence.High),
        ...
        var wall = SyntheticWall("confirmed-main-interior", 180, 180, 310, 180) with
        {
            DetectionKind = WallDetectionKind.ParallelLinePair,
            WallType = WallType.Interior,
            SourcePrimitiveIds = Enumerable.Range(1, 34).Select(index => $"confirmed-risk-source-{index}").ToArray(),
            Evidence =
            [
                "parallel wall-face pair",
                "first face merged 31 fragments",
        ...
        var wallGraph = new WallGraph(
            [
                SyntheticNode("n1", 180, 180),
                SyntheticNode("n2", 310, 180)
            ],
            [
                SyntheticEdge("e1", "n1", "n2", wall.Id)
            ],
        ...
        var result = CreateSyntheticResult(
            ...
            wallEvidenceMap: new WallEvidenceMap(
                ...
                [],
                [
                    StrongPairedWallEvidence(wall, "both endpoints supported by structural context")
                ])) with
        {
            Quality = UsableQuality()
        };
    ...
    public void PlacementExporter_BlocksReviewOnlyTwoSidedFragmentMergedSecondaryRoomBoundaryWithoutExplicitRoomLink()
    {
        ...
        var wall = SyntheticWall("two-sided-fragment-secondary", 180, 180, 180, 292) with
        {
            DetectionKind = WallDetectionKind.FragmentMerged,
            WallType = WallType.Interior,
            SourcePrimitiveIds = Enumerable.Range(1, 4).Select(index => $"two-sided-fragment-source-{index}").ToArray(),
            FragmentEvidence = new WallFragmentEvidence(
                4,
                0,
                0,
                ...
                0,
                RequiresGeometryReview: false,
                ["fragment geometry: 4 fragment(s)", "fragment geometry healed gap ratio 0"]),
            Evidence =
            [
                "merged collinear wall fragments",
        ...
        var roomAnchor = SyntheticWall("room-boundary-anchor", 260, 100, 380, 100);
        var wallGraph = new WallGraph(
            [SyntheticNode("n1", 180, 180), SyntheticNode("n2", 180, 292)],
            [SyntheticEdge("e1", "n1", "n2", wall.Id)],
            [
                new WallGraphComponent(
                    "two-sided-fragment-secondary-component",
                    1,
                    WallGraphComponentKind.SecondaryStructural,
                    new PlanRect(176, 180, 8, 112),
                    [wall.Id],
                    ["n1", "n2"],
                    ["e1"],
                    [wall.Id],
                    wall.DrawingLength,
                    Confidence.High,
                    ["long interior fragment recovered as secondary structural wall context"])
            ]);
        var assessment = new WallEvidenceWallAssessment(
            wall.Id,
            wall.PageNumber,
            ...
            wall.Evidence)
        {
            ...
            ScoreBreakdown = new WallEvidenceScoreBreakdown(
                ...
                0,
                0,
                ["medium wall-body geometry", "both endpoints supported by structural context"],
                ["not placement-ready without review"])
        };
        var result = CreateSyntheticResult(
            regions: regions,
            walls: [wall, roomAnchor],
            wallGraph: wallGraph,
            rooms: [SyntheticRoom("r1", new PlanRect(260, 100, 120, 120), [roomAnchor.Id])],
            wallEvidenceMap: new WallEvidenceMap(
                [],
                [],
                [
    ...
    [Fact]
    public void PlacementExporter_AllowsGraphPromotedLongOneEndpointSecondaryFragment()
    {
        var regions = new[]
        {
            new SheetRegion("sheet", 1, RegionKind.Sheet, new PlanRect(0, 0, 600, 400), Confidence.High),
        ...
        var wall = SyntheticWall("graph-promoted-one-end-fragment", 180, 180, 180, 292) with
        {
            DetectionKind = WallDetectionKind.FragmentMerged,
            WallType = WallType.Interior,
            SourcePrimitiveIds = Enumerable.Range(1, 4).Select(index => $"one-end-fragment-source-{index}").ToArray(),
            FragmentEvidence = new WallFragmentEvidence(
                4,
                0,
                0,
            ...
            [
                ...
                "run merged 4 fragments",
                "wall type interior: supported wall evidence inside exterior envelope",
                "wall evidence assessment: long interior fragment promoted to placement-ready by secondary structural graph component secondary-fragment-component"
            ]
        };
        var wallGraph = new WallGraph(
            [SyntheticNode("n1", 180, 180), SyntheticNode("n2", 180, 292)],
        ...
        var assessment = new WallEvidenceWallAssessment(
            ...
            [
                ...
                "wall type interior: supported wall evidence inside exterior envelope",
                "wall evidence: unlayered fragment-merged wall candidate has only one trusted structural endpoint (4 fragments, gap ratio 0); keep for topology but block exact placement until reviewed",
                "wall evidence: promoted to placement-ready by secondary structural graph component secondary-fragment-component",
            ...
            ])
        {
            ...
            ScoreBreakdown = new WallEvidenceScoreBreakdown(
                ...
                0,
                0,
                ["medium wall-body geometry", "one endpoint supported by structural context", "secondary structural interior fragment continuity"],
                [])
        };
        var result = CreateSyntheticResult(
            regions: regions,
            ...
            wallGraph: wallGraph,
            rooms: [],
            wallEvidenceMap: new WallEvidenceMap([], [], [assessment])) with
        {
            Quality = UsableQuality()
        };
        ...
        Assert.False(
            document.RootElement.GetProperty("summary").GetProperty("wallPlacementOmissionCounts")
                .TryGetProperty("one_endpoint_fragment_review_required", out _));
    }
