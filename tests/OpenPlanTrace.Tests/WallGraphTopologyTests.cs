...
public sealed class WallGraphTopologyTests
{
    ...
    [Fact]
    public async Task WallGraphStage_PromotesContinuousDenseInteriorFragmentComponentWithGraphContext()
    {
        var mainWalls = new[]
        {
            DetectedWall("main-top", new PlanPoint(80, 240), new PlanPoint(260, 240)) with { WallType = WallType.Exterior },
            DetectedWall("main-right", new PlanPoint(260, 240), new PlanPoint(260, 360)) with { WallType = WallType.Exterior },
            DetectedWall("main-bottom", new PlanPoint(260, 360), new PlanPoint(80, 360)) with { WallType = WallType.Exterior },
            DetectedWall("main-left", new PlanPoint(80, 360), new PlanPoint(80, 240)) with { WallType = WallType.Exterior }
        };
        var denseFragment = DenseContinuousFragmentWall(
            "wall-dense-continuous-fragment",
            new PlanPoint(450, 90),
            new PlanPoint(450, 230),
            includeInteriorSupport: true);
        var context = new ScanContext(
            Document("wall-dense-continuous-fragment-recovery"),
            new ScannerOptions());
        context.Walls.AddRange(mainWalls.Append(denseFragment));
        context.WallEvidenceMap = new WallEvidenceMap(
            Array.Empty<WallEvidenceSegment>(),
            Array.Empty<WallEvidenceBand>(),
            mainWalls
                .Select(wall => Assessment(wall, WallEvidenceDecision.Accept, WallEvidenceCategory.StrongWallBody, Confidence.High))
                .Append(DenseContinuousFragmentReviewAssessment(denseFragment, includeInteriorSupport: true))
                .ToArray());
        ...
        var fragmentComponent = Assert.Single(
            context.WallGraph.Components,
            component => component.WallIds.Contains(denseFragment.Id));
        var retainedReview = Assert.Single(context.WallEvidenceMap.WallAssessments, assessment => assessment.WallId == denseFragment.Id);
        var retainedWall = Assert.Single(context.Walls, wall => wall.Id == denseFragment.Id);

        Assert.Equal(WallGraphComponentKind.SecondaryStructural, fragmentComponent.Kind);
        Assert.False(fragmentComponent.ExcludedFromStructuralTopology);
        Assert.Contains(fragmentComponent.Evidence, item => item.Contains("long continuous fragment recovered", StringComparison.OrdinalIgnoreCase));
        Assert.True(retainedReview.PlacementReady);
        Assert.False(retainedReview.RequiresReview);
        Assert.Equal(WallEvidenceDecision.Accept, retainedReview.Decision);
        Assert.Contains(
    ...
    [Fact]
    public async Task WallGraphStage_KeepsDenseFragmentWithoutInteriorSupportReviewOnly()
    {
        var mainWalls = new[]
        {
            DetectedWall("main-top", new PlanPoint(100, 80), new PlanPoint(520, 80)) with { WallType = WallType.Exterior },
            DetectedWall("main-right", new PlanPoint(520, 80), new PlanPoint(520, 320)) with { WallType = WallType.Exterior },
            DetectedWall("main-bottom", new PlanPoint(520, 320), new PlanPoint(100, 320)) with { WallType = WallType.Exterior },
            DetectedWall("main-left", new PlanPoint(100, 320), new PlanPoint(100, 80)) with { WallType = WallType.Exterior }
        };
        var denseFragment = DenseContinuousFragmentWall(
            "wall-dense-unsupported-fragment",
            new PlanPoint(330, 105),
            new PlanPoint(330, 245),
            includeInteriorSupport: false);
        var context = new ScanContext(
            Document("wall-dense-unsupported-fragment-review"),
            new ScannerOptions());
        context.Walls.AddRange(mainWalls.Append(denseFragment));
        context.WallEvidenceMap = new WallEvidenceMap(
            Array.Empty<WallEvidenceSegment>(),
            ...
            mainWalls
                .Select(wall => Assessment(wall, WallEvidenceDecision.Accept, WallEvidenceCategory.StrongWallBody, Confidence.High))
                .Append(DenseContinuousFragmentReviewAssessment(denseFragment, includeInteriorSupport: false))
                .ToArray());
        ...
        var fragmentComponent = Assert.Single(
            context.WallGraph.Components,
            component => component.WallIds.Contains(denseFragment.Id));
        var assessment = Assert.Single(context.WallEvidenceMap.WallAssessments, item => item.WallId == denseFragment.Id);
    ...
    [Fact]
    public async Task WallGraphStage_SuppressesWeakDetailEndpointGapAgainstTrustedHostWall()
    {
        var top = DetectedWall("wall-room-top", new PlanPoint(100, 100), new PlanPoint(320, 100));
        var right = DetectedWall("wall-room-right", new PlanPoint(320, 100), new PlanPoint(320, 260));
        var bottom = DetectedWall("wall-room-bottom", new PlanPoint(320, 260), new PlanPoint(100, 260));
        var left = DetectedWall("wall-room-left", new PlanPoint(100, 260), new PlanPoint(100, 100));
        var weakDetail = DetectedWall("wall-weak-detail-endpoint", new PlanPoint(200, 114), new PlanPoint(200, 220)) with
        {
            Confidence = new Confidence(0.64),
        ...
        var context = new ScanContext(
            Document("wall-weak-detail-endpoint-gap-suppression"),
            new ScannerOptions());
        context.Walls.AddRange(new[] { top, right, bottom, left, weakDetail });
        context.WallTopologyPreparation = new WallTopologyPreparation(
            new[] { bottom.Id, left.Id, right.Id, top.Id, weakDetail.Id },
            Array.Empty<WallTopologyRejectedWall>(),
            new[] { bottom.Id, left.Id, right.Id, top.Id, weakDetail.Id },
            Array.Empty<string>(),
            Array.Empty<string>());
        ...
        Assert.Contains(
            context.Diagnostics.Build().Messages,
            diagnostic => diagnostic.Code == "wall_graph.endpoint_gap.weak_fragment_suppressed"
                && diagnostic.Properties["suppressedEndpointGapCandidateCount"] == "1");
    }

    [Fact]
    public async Task WallGraphStage_SuppressesEndpointGapRepairCandidateAcrossKnownOpening()
    {
        var hostWall = DetectedWall("wall-opening-host", new PlanPoint(100, 60), new PlanPoint(100, 180))
            with
            {
                DetectionKind = WallDetectionKind.ParallelLinePair,
        ...
        var partition = DetectedWall("wall-opening-gap-partition", new PlanPoint(116, 100), new PlanPoint(240, 100))
            with
            {
                ...
                Evidence = new[]
                {
                    ...
                    "wall evidence assessment: MediumWallBody / placement-ready / confidence 0.66"
                }
            };
        var opening = new OpeningCandidate(
            ...
            1,
            OpeningType.Door,
            new PlanRect(86, 76, 54, 54),
            new Confidence(0.80))
        {
            WallId = hostWall.Id,
            HostWallIds = new[] { hostWall.Id },
            CenterLine = new PlanLineSegment(new PlanPoint(100, 100), new PlanPoint(116, 100)),
            Evidence = new[] { "door swing arc attached to host wall" }
        };
        var context = new ScanContext(
            Document("wall-opening-gap-repair-suppression"),
            new ScannerOptions());
        ...
        Assert.Contains(
            context.Diagnostics.Build().Messages,
            diagnostic => diagnostic.Code == "wall_graph.endpoint_gap.opening_suppressed"
                && int.Parse(
                    diagnostic.Properties["suppressedEndpointGapCandidateCount"],
    ...
    [Fact]
    public async Task WallGraphStage_AutoSnapsTrustedMediumPairedEndpointGapAcrossReviewTolerance()
    {
        var hostWall = DetectedWall("wall-trusted-medium-host", new PlanPoint(100, 60), new PlanPoint(100, 180))
            with
            {
                DetectionKind = WallDetectionKind.ParallelLinePair,
                Confidence = new Confidence(0.91),
                PairEvidence = new WallPairEvidence(
                    new PlanLineSegment(new PlanPoint(96, 60), new PlanPoint(96, 180)),
                    new PlanLineSegment(new PlanPoint(104, 60), new PlanPoint(104, 180)),
                    FaceSeparation: 8,
                    OverlapRatio: 1,
                    Score: 0.90,
                    FirstFaceFragmentCount: 3,
                    SecondFaceFragmentCount: 3,
                    FirstFaceSourcePrimitiveIds: ["wall-trusted-medium-host-face-a"],
                    SecondFaceSourcePrimitiveIds: ["wall-trusted-medium-host-face-b"]),
                Evidence = new[]
                {
                    "parallel wall-face pair",
                    "pair score 0.90",
        ...
        var partition = DetectedWall("wall-trusted-medium-gap-partition", new PlanPoint(116.5, 100), new PlanPoint(240, 100))
            with
            {
                DetectionKind = WallDetectionKind.ParallelLinePair,
                Confidence = new Confidence(0.90),
                WallType = WallType.Interior,
                PairEvidence = new WallPairEvidence(
                    new PlanLineSegment(new PlanPoint(116.5, 96), new PlanPoint(240, 96)),
                    new PlanLineSegment(new PlanPoint(116.5, 104), new PlanPoint(240, 104)),
                    FaceSeparation: 8,
                    OverlapRatio: 1,
                    Score: 0.69,
                    FirstFaceFragmentCount: 30,
                    SecondFaceFragmentCount: 4,
                    FirstFaceSourcePrimitiveIds: ["wall-trusted-medium-gap-partition-face-a"],
                    SecondFaceSourcePrimitiveIds: ["wall-trusted-medium-gap-partition-face-b"]),
                Evidence = new[]
                {
                    "parallel wall-face pair",
                    "pair score 0.69",
                    "overlap ratio 1",
                    "first face merged 30 fragments",
                    "wall type interior: supported wall evidence inside exterior envelope",
                    "wall evidence: short unlayered parallel-face candidate has noisy fragmented face evidence; keep for topology but block exact placement until reviewed",
        ...
        var context = new ScanContext(
            Document("wall-trusted-medium-paired-endpoint-gap-full-review-auto-snap"),
            new ScannerOptions());
        context.Walls.AddRange(new[] { hostWall, partition });
        context.WallTopologyPreparation = new WallTopologyPreparation(
            new[] { hostWall.Id, partition.Id },
        ...
        context.WallEvidenceMap = new WallEvidenceMap(
            ...
            new[]
            {
                Assessment(hostWall, WallEvidenceDecision.Accept, WallEvidenceCategory.StrongWallBody, new Confidence(0.91)),
                MediumPairedReviewAssessmentWithPairScore(partition, 0.69) with
                {
                    Confidence = new Confidence(0.90),
                    PlacementReady = true,
        ...
        Assert.Equal(100, normalizedPartition.CenterLine.Start.Y, precision: 1);
        Assert.DoesNotContain(context.WallGraph.Nodes, node =>
            node.Position.DistanceTo(new PlanPoint(116.5, 100)) <= 0.5);
        Assert.Contains(
            context.Diagnostics.Build().Messages,
            diagnostic => diagnostic.Code == "wall_graph.endpoint_gap.trusted_endpoint_snapped"
                && diagnostic.Properties["trustedEndpointSnapCount"] == "1");
    ...
    private static WallEvidenceWallAssessment DenseContinuousFragmentReviewAssessment(
        WallSegment wall,
        bool includeInteriorSupport)
    {
        var evidence = new List<string>
        {
            "merged collinear wall fragments",
            "run merged 99 fragments",
            "run collapsed 2 duplicate primitives",
            "layer (unlayered) classified Dimension (0.24)"
        };
        if (includeInteriorSupport)
        {
            evidence.Add("wall type interior: supported wall evidence inside exterior envelope");
        }

        evidence.Add("wall evidence: unlayered fragment-merged wall candidate has only one trusted structural endpoint (99 fragments, gap ratio 0); keep for topology but block exact placement until reviewed");
            ...
            evidence)
        {
            ...
            ScoreBreakdown = new WallEvidenceScoreBreakdown(
                ...
                0,
                0,
                includeInteriorSupport ? 0.20 : 0.04,
                0.10,
                0,
                0.08,
                includeInteriorSupport
                    ? new[] { "fragment-merged wall body", "one trusted structural endpoint", "supported wall evidence inside exterior envelope" }
                    : new[] { "fragment-merged wall body", "one trusted structural endpoint" },
                new[] { "not placement-ready without graph recovery" })
        };
    }
    ...
    private static WallSegment DenseContinuousFragmentWall(
        string id,
        PlanPoint start,
        PlanPoint end,
        bool includeInteriorSupport)
    {
        ...
        return DetectedWall(id, start, end) with
        {
            ...
            SourcePrimitiveIds = Enumerable.Range(1, 101)
                .Select(index => $"{id}:fragment-{index.ToString("000", System.Globalization.CultureInfo.InvariantCulture)}")
                .ToArray(),
            FragmentEvidence = new WallFragmentEvidence(
                99,
                0,
                ...
                0,
                RequiresGeometryReview: false,
                new[] { "fragment geometry: 99 fragment(s)", "fragment geometry healed gap ratio 0" }),
            Evidence = evidence
        };
    }
