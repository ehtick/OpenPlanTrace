...
- wall segments, including centerline fallback, compact object-linework prefiltering that still works on weakly classified unlayered PDF content, orthogonal and non-orthogonal duplicate/fragmented collinear wall cleanup, gap-healed fragment runs, and parallel-line wall-pair reconstruction with structured face-pair evidence
...
- placement wall reliability reasons for object-like wall components, isolated wall graph fragments, topology-excluded detail linework, surface/detail overlaps, low-confidence walls, and missing metric scale, so downstream engines can retain exact coordinates without treating every exported wall as equally trusted structural geometry
...
GeoJSON coordinates are OpenPlanTrace page drawing units, not WGS84 longitude/latitude. The export carries `schemaVersion=openplantrace.geojson.v1`, `coordinateSpace=OpenPlanTracePageCoordinates`, and feature properties for page number, detector type, confidence, labels, source primitive IDs, source layers, surface-pattern exclusion policy, routing influence, room-use hints, child object suppression, and measurements when available. The full scan JSON also includes a top-level `coordinateSystem` block with page bounds, axes, coordinate order, normalized transforms, calibration linkage, and `routingLayer` data for downstream software.
...
The provided-PDF fixtures are marked `optional`, so missing local PDFs are reported as skipped cases instead of failed benchmarks. Fixture `sourcePath` values support environment variables such as `%USERPROFILE%`; present optional files are scanned normally and must satisfy their expectations.

Draft a measured benchmark manifest from an exported scan JSON:

```powershell
dotnet run --project .\tools\OpenPlanTrace.Cli\OpenPlanTrace.Cli.csproj -- benchmark-draft .\scan-output\scan.json --source-path "%USERPROFILE%\Downloads\Blyverket A20-1 Plan 1. etasje.pdf" --fixture-id blyverket-a20-plan-1 --json .\samples\provided-pdfs\blyverket-a20-plan-1.draft.json --review-markdown .\samples\provided-pdfs\blyverket-a20-plan-1.draft-review.md
```
