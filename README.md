...
- exact reviewable circular wall-pair evidence from compatible native arcs or fitted PDF polyline faces, with center, centerline radius, start angle, signed sweep, thickness, endpoints, bounds, source IDs, fit error, confidence, and calibrated measurements when available; these candidates remain excluded from line-only topology and coordinate placement until mixed line/curve graph solving is implemented
...
- page-coordinate GeoJSON feature export for downstream QA/mapping tools, including pages, regions, surface patterns, walls, reviewable curved walls, wall nodes, rooms, room adjacencies, room clusters, openings, grid axes, grid bay spacings, dimensions, annotations, annotation references, objects, object groups, object aggregates, and routing-layer features; circular walls use standards-valid sampled `LineString` geometry while retaining exact canonical center/radius/angle properties and an explicit approximation marker
