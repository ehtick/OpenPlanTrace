...
Current alpha version: `0.12.005`.
...
- wall candidate detection followed by a dedicated wall evidence refinement stage, including centerline fallback, axis-aligned and rotated filled-wall body recovery, source-backed physical wall-body median anchoring, compact object-linework and door/detail symbol prefiltering that still works on weakly classified unlayered PDF content, conservative dimension-like fragment-run rejection, orthogonal and non-orthogonal duplicate/fragmented collinear wall cleanup, gap-healed fragment runs, parallel-line wall-pair reconstruction with structured face-pair evidence, wall evidence segments/bands, exported per-wall evidence assessments, placement-readiness assessment, opt-in guarded missing wall-band recovery/noise rejection, and explicit rejected-noise diagnostics
...
- first-class non-structural surface/detail pattern candidates for dense orthogonal grids, parallel bands, and two-family diagonal cross-hatches, such as terrace, hatch, overroof, or tile-like linework, with page bounds, source primitive IDs/layers, spacing/intersection evidence, confidence, review flags, and explicit exclusion from wall detection and structural topology
- exact reviewable circular wall-pair evidence from compatible native arcs or fitted PDF polyline faces, with center, centerline radius, start angle, signed sweep, thickness, endpoints, bounds, source IDs, fit error, confidence, and calibrated measurements when available; scan and structure outputs preserve them as mixed structural paths with endpoint-only tangent/corner relations, while curved paths and junctions remain excluded from coordinate placement
...
- schema-versioned JSON export with a portable page coordinate-system contract, source document provenance, title-block, dimension, annotation item references, surface patterns, exact circular wall-pair evidence, grid-axis, grid-bay spacing, deterministic room-use, room-adjacency, typed room-cluster semantic evidence, routing-layer barriers/passages/obstacles/room-use hints/suppressed objects/ignored objects, graph-junction-aware routing barriers with minor junction compression, suppression of unused isolated wall fragments and dense secondary detail patterns from trusted routing barriers, source primitive IDs, and source layers
- schema-versioned canonical structure export with globally solved wall runs, canonical nodes, rooms, openings, exact page and millimeter coordinates, wall type/thickness, source wall/graph/primitive provenance, mixed straight/circular path topology, reliability, integrity metrics, and actionable structural issues
...
- page-coordinate GeoJSON feature export for downstream QA/mapping tools, including pages, regions, surface patterns, walls, reviewable curved walls, mixed-path junctions, wall nodes, rooms, room adjacencies, room clusters, openings, grid axes, grid bay spacings, dimensions, annotations, annotation references, objects, object groups, object aggregates, and routing-layer features; circular walls use standards-valid sampled `LineString` geometry while retaining exact canonical center/radius/angle/path properties, and junction `MultiPoint` features preserve both source endpoints instead of inventing snapped geometry
...
The structure contract is `openplantrace.structure.v2`. Export its schema with
`openplantrace schema structure --out .\openplantrace.structure.v2.schema.json`
...
lengths, topology references, mixed path geometry/junction integrity, and
summary totals. Structure v2 adds discriminated `Line` and `CircularArc` paths,
stable source-backed path IDs, exact arc parameters, and review-only endpoint
relations. Existing placement JSON remains straight-wall-only and does not
promote curved evidence into trusted coordinates.
